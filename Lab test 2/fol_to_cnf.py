        ...
        for i in range(len(self.my_stack)):
            ...
            for j in re.findall(reg0, target):
                ...
                if logic not in child:
                    continue
                new_reg = "(^|\s)" + j + "(\s|$)"
                self.my_stack[i] = re.sub(
                    new_reg, ' ' + child + ' ', self.my_stack[i], count=1)
...
class de_morgan(logic_base):
    def run(self):
        ...
        for i in range(len(self.my_stack)):
            ...
            self.my_stack[i] = re.sub(
                reg, str(len(self.my_stack)), target, count=1)
        ...
        for i in range(len(self.my_stack)):
            ...
            for j in re.findall(reg, target):
                ...
                tmp_list = [str(j) for j in range(
                    len(self.my_stack), len(self.my_stack) + len(items))]
                for item in items:
                    self.my_stack.append(
